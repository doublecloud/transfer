...
var (
	...
	Target, targetConn = helpers.RecipeMysqlTargetWithConnection("target_conn_id")
)
...
func Load(t *testing.T) {
	...
	cfg.Addr = fmt.Sprintf("%v:%v", srcConn.Hosts[0].Name, srcConn.Hosts[0].Port)
	cfg.User = srcConn.User
	cfg.Passwd = string(srcConn.Password)
