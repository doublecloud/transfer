...
func Load(t *testing.T) {
	...
	createQuery := "create table IF NOT EXISTS __test (a_id integer not null primary key, max_weight numeric);"
	...
	//------------------------------------------------------------------------------
	// wait while replicated

	insertQuery := "INSERT INTO public.__test (a_id, max_weight) VALUES (7, 30);"
	_, err = srcConn.Exec(context.Background(), insertQuery)
	require.NoError(t, err)

	tableDescription := abstract.TableDescription{Name: "__test"}
	tablePaths := []ypath.Path{
		ypath.Path(Target.Path()).Child(getTableName(tableDescription)),
	}
	...
	require.NoError(t, err)
	helpers.CanonizeTableChangeItems(t, storage, tableDescription)
}
