...
func TestAuditTrailsV1(t *testing.T) {
	...
	dst := &opensearch.OpenSearchDestination{}
	...
	sink, err := opensearch.NewSinkImpl(
		dst,
		logger.Log,
	...
	helpers.WriteToLb(t, lbEnv.ProducerOptions().Endpoint, lbEnv.DefaultTopic, lbSendingPort, logbroker.LbDestination{Credentials: lbEnv.Creds}, lines)
