...
func TestReplication(t *testing.T) {
	...
	syncBinlogPosition := func() {
		err := mysqlSource.SyncBinlogPosition(Source, transfer1.ID, fakeClient)
	...
	ytTestPath := "//home/cdc/test/mysql2lb2yt_all_types"
	...
	worker1 := startWorker(t, transfer1, fakeClient)
	defer stopWorker(worker1)

	worker2 := startWorker(t, transfer2, coordinator.NewFakeClient())
	defer stopWorker(worker2)
