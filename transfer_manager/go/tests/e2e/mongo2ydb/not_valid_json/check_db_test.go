...
import (
	...
	ydbStorage "github.com/doublecloud/transfer/transfer_manager/go/pkg/providers/ydb"
...
func Load(t *testing.T) {
	...
	_, err = coll.InsertOne(context.Background(), bson.M{"a": 1, "aa": math.NaN()})
	require.NoError(t, err)

	err = db.CreateCollection(context.Background(), "test_excl")
	require.NoError(t, err)
	exclCol := db.Collection("test_excl")
	_, err = exclCol.InsertOne(context.Background(), bson.M{"a": 2, "aa": math.NaN()})
	require.NoError(t, err)
	//------------------------------------------------------------------------------------
	// start worker
	...
	_, err = coll.InsertOne(context.Background(), bson.M{"b": 1, "aa": math.NaN()})
	require.NoError(t, err)

	_, err = exclCol.InsertOne(context.Background(), bson.M{"b": 2, "aa": math.NaN()})
	require.NoError(t, err)
